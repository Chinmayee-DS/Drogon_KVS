...
namespace v1
{
class controne : public drogon::HttpController<controne>
{
  public:
    ...
    // ADD_METHOD_VIA_REGEX(TestCtrl::getItem, "/{key}", Get);
        ADD_METHOD_TO(controne::getItem,"/{key}",Get);
        ADD_METHOD_TO(controne::putItem, "/{key}", Post);
        ADD_METHOD_TO(controne::delItem, "/{key}", Delete);
